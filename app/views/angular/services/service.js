...
myApp.service("cartService",function($http){
	...
	this.updatePasswordApi = function(info){
		...
		// ADD VENDORS
	this.addVendorApi = function(data){
		return $http.post('/user/addVendor',data);
	}
		// ADD VENDORS
	this.deleteVendorApi = function(id){
		return $http.get('/user/deleteVendor/'+id+'');
	}
	
		// GET ALL VENDORS
	this.getAllVendorsApi = function(type){
		return $http.get('/user/getAllVendors/'+type+'');
	}
		// CONFIRM ORDER
	this.confirmOrderApi = function(){
		return $http.get('/orders/confirmOrder');
	}
	// CONFIRM ORDER
	this.getAllOrderApi = function(){
		return $http.get('/orders/allOrders');
	}
		// CONFIRM ORDER
	this.cancelOrderApi = function(id){
		return $http.get('/orders/cancelOrder/'+id+'');
	}
})