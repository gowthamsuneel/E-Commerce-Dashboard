...
myApp.controller("DashController",["$http",'$location','cartService','$rootScope','SweetAlert',
	...
	main.viewVendors = true;
	main.addProduct = true;
	main.viewCustomers=true;

	this.showForm = false;

	this.getDashboard = function(){
		
         if($rootScope.roleAccess == 'vendor' || $rootScope.roleAccess == 'customer' ){
        main.viewVendors = false;
        main.viewCustomers=false;
        	if($rootScope.roleAccess == 'customer'){
        		main.addProduct = false;
        	}
         }
		...
		.then(function successCallback(response){
			...
			if(response.data.userLog == false){
				...
				cartService.userEmail = dashData.email;
