...
myApp.controller("SignupController",["$http",'$location','cartService','SweetAlert','$rootScope',
	function($http,$location,cartService,SweetAlert,$rootScope){
		...
		.then(function successCallback(response){
			...
			if(response.data.loggedIn == true || response.data.loggedIn != undefined){
				...
				if(response.data.status == 200 && response.data.data.alreadyPresent !== true){
					$rootScope.roleAccess = response.data.data.role;
